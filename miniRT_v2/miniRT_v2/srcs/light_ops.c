...
static	double	calc_specular(t_vector_2p ray, t_inter *inter, t_scene scene, t_figure *lst)
{
	double		light;
	t_vector	direction;
	t_vector	to_cam;
	...
	t_scene scene, t_figure *lst)
{
	...
	while (scene.light)
	{
		...
		if (in_light(scene.light->origin, inter->point, vector_mlt(-1, direction), lst)
			&& vector_dot(inter->normal, direction) > 0)
		{
			light = scene.light->brightness * vector_cos(inter->normal, direction);
