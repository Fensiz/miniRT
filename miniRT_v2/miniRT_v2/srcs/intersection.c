...
static double	caps_cone_intersection(t_vector o, t_vector d, t_figure *lst)
{
	double	id1;
	t_vector	ip1;
...
double cone_solve(t_vector origin, t_vector direction, t_figure *lst)
{
	...
	double a = pow(vector_dot(direction, v), 2) - pow(cos(theta), 2);
	double b = 2. * (vector_dot(direction, v) * vector_dot(co, v) - vector_dot(direction, co) * pow(cos(theta), 2));
	double c = pow(vector_dot(co, v), 2) - vector_dot(co,co) * pow(cos(theta), 2);
	...
	double t = t1;
	if (t < EPSILON || (t2 > EPSILON && t2 < t)) t = t2;
	if (t < EPSILON) return (INFINITY);

	t_vector cp = vector_sub(vector_sum(origin, vector_mlt(t, direction)), center);
	double h = vector_dot(cp, v);
	if (h < EPSILON || h > lst->figure.co.height) return (INFINITY);

	lst->normal = vector_norm(vector_sub(vector_mlt(vector_dot(v, cp) / vector_dot(cp, cp), cp), v));
	return t;
...
double cone_intersection(t_vector origin, t_vector direction, t_figure *lst)
{
	...
	cone_inter = cy_intersection(origin, direction, &co_normal, lst);
	cap_inter = caps_intersection(origin, direction, lst);
